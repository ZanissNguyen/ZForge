...
import org.zanissnguyen.ZForge.Listeners.UX.UX_material;
import org.zanissnguyen.ZForge.System.object.zmaterial;

public class UI_material extends UI_list
{
	public final String permission = "zforge.material";
	public static String name_prefix;
	public ItemStack laner = null;
	public ItemStack fill_slot = null;
	
	public UI_material(ZForge plugin) {
		super(plugin, plugin.utils, plugin.file_loc.getForge("material.ui-name"), 6);
		
		name_prefix = plugin.file_loc.getForge("material.ui-name");
		...
		new UX_material(plugin, utils, this);
	}

	public ItemStack loadIcon(zmaterial item, boolean tips)
	{
...
//		int maxPage = items.size()/36+1;
	...
	public List<ItemStack> loadList(Player p, String search, int page)
	{
		...
		List<zmaterial> materials = new ArrayList<>();
		for (String s: plugin.file_mat.searchFor(search))
		{
			materials.add(plugin.file_mat.getMaterial(s));
		}
		
		List<ItemStack> itemStacks = new ArrayList<>();
		for (zmaterial mat: materials)
		{
			itemStacks.add(loadIcon(mat, showTips));
		}
