...
public class UX_material extends zevent
{
	public List<Player> onSearch;
	
	private UI_material ui;
	public UX_material(ZForge plugin, utils utils, UI_material ui) {
		super(plugin, utils);
		this.ui = ui;
...
//			int maxPage = Integer.parseInt(maxPageStr);
			...
			if (slot==50 && clicked.getItem(slot).isSimilar(ui.findIcon(keyword, plugin.file_mat.searchFor(keyword).size())))
			{
				p.closeInventory();
				onSearch.add(p);
			...
			if ((slot>=0 && slot<=35) && !( item.isSimilar(ui.fillIcon()) || item.isSimilar(ui.fill_slot) || item.isSimilar(ui.laner) ))
			{
				if (utils.hasPermission(p, ui.permission)) 
				{
					...
					if (name.contains("#"))
					{
						...
						zmaterial got = plugin.file_mat.getMaterial(id);
